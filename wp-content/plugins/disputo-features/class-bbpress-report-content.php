...
class bbp_ReportContent {
	...
	public static function activation_check() {
	    if ( ! self::is_bbpress_active() ) {
	        ...
	        wp_die( __( 'bbPress - Report Content requires bbPress to be activated.', 'disputo' ) );
	    }
	}
	...
	public static function disabled_notice() {
		echo '<div class="updated"><p><strong>' . esc_html__( 'bbPress - Report Content was deactivated. This plugin requires bbPress to be active.', 'disputo' ) . '</strong></p></div>';
	}
	...
	public function register_post_status() {
		...
		register_post_status(
			...
			apply_filters( 'bbp_rc_register_reported_post_status', array(
				'label'                     => _x( 'User Reported', 'post', 'disputo' ),
				'label_count'               => _nx_noop( 'User Reported <span class="count">(%s)</span>', 'User Reported <span class="count">(%s)</span>', 'post', 'disputo' ),
				'public'                    => true,
	...
	public function add_topic_status( $statuses ) {
		...
		$statuses[$this->get_reported_status_id()] = _x( 'Reported', 'Mark topic as reported', 'disputo' );
	...
	function get_topic_report_link( $args = '' ) {
		...
		$r = bbp_parse_args( $args, array(
			...
			'link_after'   => '',
			'sep'          => ' | ',
			'report_text'    => esc_html__( 'Report',   'disputo' ),
			'unreport_text'  => esc_html__( 'Unreport', 'disputo' )
		...
		$title = ( true === $reported ) ? __( 'Unreport inappropriate content', 'disputo' ) : __( 'Report inappropriate content', 'disputo' );
		if ( true === $reported ) {
			$classes[] = 'reported';
		} else {
			$classes[] = 'unreported';
	...
	public function toggle_topic_handler( $action = '' ) {
		...
		switch ( $action ) {
			...
			case 'bbp_rc_toggle_topic_report' :
				...
				$failure  = true === $is_reported ? __( '<strong>ERROR</strong>: There was a problem unmarking the topic as reported.', 'disputo' ) : __( '<strong>ERROR</strong>: There was a problem reporting the topic.', 'disputo' );
	...
	public function insert_report_status( $r ) {
        ...
        if (is_array($r['post_status'])) {
            array_push($r['post_status'], $this->get_reported_status_id());
        } else {
	...
	public function toggle_reply_handler( $action = '' ) {
		...
		switch ( $action ) {
			...
			case 'bbp_rc_toggle_reply_report' :
				...
				$failure  = true === $is_reported ? __( '<strong>ERROR</strong>: There was a problem unmarking the reply as reported.', 'disputo' ) : __( '<strong>ERROR</strong>: There was a problem reporting the reply.', 'disputo' );
				// $view_all = !$is_reported; // Only need this if we want to hide it, like spam
	...
	public function output_topic_notice() {
		...
		echo '<div class="bbp-template-notice error bbp-rc-topic-is-reported">';
			echo '<p>';
				echo apply_filters( 'bbp_rc_topic_notice', __( 'This topic has been reported for inappropriate content', 'disputo' ) );
			echo '</p>';
		echo '</div>';
	}
	...
	public function output_reply_notice() {
		...
		echo '<div class="error bbp-rc-reply-is-reported">';
			echo '<div class="alert alert-danger">';
				echo apply_filters( 'bbp_rc_reply_notice', __( 'This reply has been reported for inappropriate content.', 'disputo' ) );
			echo '</p>';
		echo '</div>';
	}
	...
	public function topic_row_actions( $actions, $topic ) {
		...
		if ( current_user_can( 'moderate', $topic->ID ) ) {
			...
			if ( $this->is_topic_reported( $topic->ID ) )
				$actions['report'] = '<a href="' . esc_url( $report_uri ) . '" title="' . esc_attr__( 'Mark the topic as unreported', 'disputo' ) . '">' . esc_html__( 'Unreport', 'disputo' ) . '</a>';
		}
	...
	public function toggle_topic_admin_handler() {
		...
		if ( bbp_is_get_request() && !empty( $_GET['action'] ) && in_array( $_GET['action'], array( 'bbp_rc_toggle_topic_report' ) ) && !empty( $_GET['topic_id'] ) ) {
			...
			if ( empty( $topic ) )
				wp_die( __( 'The topic was not found!', 'disputo' ) );

			if ( !current_user_can( 'moderate', $topic->ID ) ) // What is the user doing here?
				wp_die( __( 'You do not have the permission to do that!', 'disputo' ) );
	...
	public function toggle_topic_notice_admin() {
		...
		if ( bbp_is_get_request() && !empty( $_GET['bbp_topic_toggle_notice'] ) && in_array( $_GET['bbp_topic_toggle_notice'], array( 'unreported' ) ) && !empty( $_GET['topic_id'] ) ) {
			...
			switch ( $notice ) {
				case 'unreported'    :
					$message = $is_failure === true ? sprintf( __( 'There was a problem unreporting the topic "%1$s".', 'disputo' ), $topic_title ) : sprintf( __( 'Topic "%1$s" successfully unreported.', 'disputo' ), $topic_title );
					break;
			}
	...
	public function reply_row_actions( $actions, $reply ) {
		...
		if ( current_user_can( 'moderate', $reply->ID ) ) {
			...
			if ( $this->is_reply_reported( $reply->ID ) )
				$actions['report'] = '<a href="' . esc_url( $report_uri ) . '" title="' . esc_attr__( 'Mark the reply as unreported', 'disputo' ) . '">' . esc_html__( 'Unreport', 'disputo' ) . '</a>';
		}
	...
	public function toggle_reply_admin_handler() {
		...
		if ( bbp_is_get_request() && !empty( $_GET['action'] ) && in_array( $_GET['action'], array( 'bbp_rc_toggle_reply_report' ) ) && !empty( $_GET['reply_id'] ) ) {
			...
			if ( empty( $reply ) )
				wp_die( __( 'The reply was not found!', 'disputo' ) );
	...
	public function toggle_reply_notice_admin() {
		...
		if ( bbp_is_get_request() && !empty( $_GET['bbp_reply_toggle_notice'] ) && in_array( $_GET['bbp_reply_toggle_notice'], array( 'unreported' ) ) && !empty( $_GET['reply_id'] ) ) {
			...
			switch ( $notice ) {
				case 'unreported'    :
					$message = $is_failure === true ? sprintf( __( 'There was a problem unreporting the reply "%1$s".', 'disputo' ), $reply_title ) : sprintf( __( 'Reply "%1$s" successfully unreported.', 'disputo' ), $reply_title );
					break;
			}
	...
	public function admin_topics_column_headers( $columns ) {
		...
		$columns['bbp_rc_user'] = __( 'Reported By', 'disputo' );
