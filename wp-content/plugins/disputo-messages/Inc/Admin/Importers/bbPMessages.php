...
class bbPMessages
{
    ...
    public function init($network_active)
    {
        ...
        $this->admin()->feedback(sprintf(
            __('<strong>bbPress Messages v2:</strong> Import older messages now from legacy data table. Click <a href="%1$s">here</a> to begin import or <a href="%2$s">dismiss</a> this notice.', 'disputo'),
    ...
    public function update()
    {
        ...
        if ( !isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'bbpm_import') ) {
            return $this->admin()->feedback(__('Error: Bad authentication!', 'disputo'), false);
        }
    ...
    public function screen()
    {
        ...
        <form method="post">
            ...
            <p><?php printf(__('Import older messages from legacy database table <code>%s</code>.', 'disputo'), $table); ?></p>

            <p><?php _e('If you have already imported messages using this tool, please dismiss this as it will not prevent duplicate records from inserting.', 'disputo'); ?></p>

            <p><?php _e('Once you click import, the plugin will begin fetching messages from older database table and appending them to the new one.', 'disputo'); ?></p>

            <p><?php _e('Please make sure not to cancel the import by pressing the escape key, or browser refresh or back button, until it is fully done.', 'disputo'); ?></p>

            <p><?php _e('Please note that this may take a while, depending on how many messages we\'re importing, just in case you were confused.', 'disputo'); ?></p>

            <p><label><input type="checkbox" name="delete_after_done" /> <?php printf(__('Delete <code>%s</code> legacy database table after import done', 'disputo'), $table); ?></label></p>

            <input type="hidden" name="_begin_import" value="1" />
            <?php wp_nonce_field('bbpm_import', '_wpnonce'); ?>
            <?php submit_button(__('Begin Import', 'disputo')); ?>
    ...
    public function extract()
    {
        ...
        if ( empty($inserted) ) {
            $this->admin()->feedback(__('Error occured, no messages were imported.', 'disputo'), false);
        } else {
            $this->admin()->feedback(sprintf(
                __('Import done: successfully imported %1$d messages from %2$d total (IDs from %3$d to %4$d)', 'disputo'),
                count($inserted),
                $raw_count,
            ...
            if ( isset($_POST['delete_after_done']) ) {
                if ( $wpdb->query("DROP TABLE {$table}") ) {
                    $this->admin()->feedback(sprintf(
                        __('Database table %s successfully deleted.', 'disputo'),
                        $table
                    ), true);
                } else {
                    $this->admin()->feedback(sprintf(
                        __('Error: Could not delete database table %s.', 'disputo'),
                        $table
                    ), false);
                }
            }
