...
class WP_Messages
{
    ...
    {
        $patterns = array(
            '/^delete_chat_([^($|\s|_)]*)$/i' => array($this, 'evtDeleteChat'),
            '/^fake_delete_chat_(.*?)_user_([^($|\s|_)]*)$/i' => array($this, 'evtFakeDeleteChat'),
            '/^delete_message_([^($|\s|_)]*)$/i' => array($this, 'evtDeleteMessage'),
            '/^fake_delete_message_([^($|\s|_)]*)_user_([0-9]+)$/i' => array($this, 'evtFakeDeleteMessage'),
            '/^notify_id_([0-9]+)_chat_id_(.*?)_sender_([0-9]+)_recipients_(.*)/i' => array($this, 'newMessageNotify'),
            '/^delete_orphaned_messages_from_chat_([^($|\s|_)]*)$/i' => array($this, 'cleanupOrphanedMessages'),
            '/^delete_orphaned_messages_from_chats_([^($|\s|_)]*)$/i' => array($this, 'bulkCleanupOrphanedMessages'),
        );
