...
class Init
{
    ...
    public function init()
    {
        ...
        // setup profile tab
        add_action('init', array($this, 'setupProfileTab'));
        // add rewrite rules
    ...
    public function parseSend()
    {
        ...
        if ( !bbpm_verify_nonce('send_message') ) {
            return bbpm_redirect($redirect, 1, 0)->withNotice(array(
                'nonce',
                __('Error: Bad authentication!', 'disputo'),
                'error'
            ));
        }
        ...
        if ( !is_object($Messages) ) {
            return bbpm_redirect($redirect, 1, 0)->withNotice(array(
                'wp_messages',
                __('Error occured processing your request.', 'disputo'),
                'error'
            ));
        }
        ...
        if ( empty($Messages->current_user) ) {
            return bbpm_redirect($redirect, 1, 0)->withNotice(array(
                'current_user',
                __('Error: Could not find a sender for this message!', 'disputo'),
                'error'
            ));
        }
        ...
        if ( !$chat_id && !empty($_POST['recipient']) ) {
            ...
            if ( !get_userdata($_recipient) ) {
                return bbpm_redirect($redirect, 1, 0)->withNotice(array(
                    'recipient',
                    __('Error: Invalid recipient specified!', 'disputo'),
                    'error'
                ));
            } else {
        ...
        if ( !trim($chat_id) ) {
            return bbpm_redirect($redirect, 1, 0)->withNotice(array(
                'chat_id',
                __('Error: Could not find a chat ID for this message!', 'disputo'),
                'error'
            ));
        }
    ...
    public function parseSingleActions()
    {
        ...
        switch ( $action ) {
            case 'delete':
                if ( $ids ) {
                    ...
                    if ( !$ids ) {
                        return bbpm_redirect($redirect_to, 1, 0)->withNotice(array(
                            'messages',
                            __('Error: Please select at least 1 message for this action!', 'disputo'),
                            'error'
                        ));
                    }
                    ...
                    return bbpm_redirect($redirect_to, 1, 0)->withNotice(array(
                        'messages',
                        __('Your messages are being deleted.', 'disputo'),
                        'success'
                    ));
                } else {
                    return bbpm_redirect($redirect_to, 1, 0)->withNotice(array(
            ...
            default:
                return bbpm_redirect($redirect_to, 1, 0)->withNotice(array(
                    'action',
                    __('Error: Invalid bulk action!', 'disputo'),
                    'error'
                ));
                break;
        }
    ...
    public function setupProfileTab()
    {
        ...
        $text = __('Messages', 'disputo');
        $uc = bbpm_messages()->getChatsUnreadCount();

        if ( $uc ) {
            $text = sprintf(__('Messages (%d)', 'disputo'), $uc);
        }
    ...
    public function parseQueryNew($wp_messages_args=null)
    {
            ...
            if ( $_POST ) {
                if ( isset($_POST['u']) ) {
                    ...
                    if ( !empty($user->ID) ) {
                        ...
                        bbpm_add_error('recipient', __('Invalid user! Please search and select a recipient below.', 'disputo'), 'error');
                    }
                }
                    ...
                    $users = new \WP_User_Query(array(
                        ...
                        'exclude' => array( bbp_get_current_user_id() )
                    ));
        ...
        if ( !empty($bbpm_recipient->ID) ) {
            if ( !bbpm_can_contact($bbpm_recipient->ID) ) {
                ...
                bbpm_add_error(
                    'recipient',
                    __('Sorry, you cannot contact this user. Please search and select a recipient below.', 'disputo'),
                    'error'
                );
            }
    ...
    public function parseQuerySingle($wp_messages_args=null)
    {
        ...
        if ( !$chat_id || !$messages->isChatRecipient($messages->current_user, $chat_id) ) {
            ...
            return bbpm_redirect(bbpm_messages_url(), 1, 0)->withNotice(array(
                '404_chat',
                __('Error: Could not find chat!', 'disputo'),
                'error'
            ));
        }
    ...
    public function updateSettingsAction($chat_id)
    {
        ...
        return bbpm_redirect(bbpm_messages_url($chat_id, $m->current_user), 1, 0)->withNotice(array(
            'updated',
            __('Settings updated successfully!', 'disputo'),
            'success'
        ));
    }
    ...
    public function deleteChatAction($chat_id)
    {
        ...
        return bbpm_redirect(bbpm_messages_url(null, $messages->current_user), 1, 0)->withNotice(array(
            'delete',
            __('Your chat is being deleted.', 'disputo'),
            'success'
        ));
    }
