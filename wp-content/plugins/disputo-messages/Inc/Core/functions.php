...
function bbpm_time_diff( $target, $before='', $after='' ) {
    ...
    $str = $str == '' ? __('a moment', 'disputo') : substr($str, 0, -2);
...
function bbpm_time_diff_i18n($unit) {
    $units = array(
        'year' => __('year', 'disputo'),
        'month' => __('month', 'disputo'),
        'day' => __('day', 'disputo'),
        'hour' => __('hour', 'disputo'),
        'minute' => __('minute', 'disputo'),
        'second' => __('second', 'disputo'),
        'years' => __('years', 'disputo'),
        'months' => __('months', 'disputo'),
        'days' => __('days', 'disputo'),
        'hours' => __('hours', 'disputo'),
        'minutes' => __('minutes', 'disputo'),
        'seconds' => __('seconds', 'disputo'),
    );
...
function bbpm_message_field($autosave='') {
    $html = '<input class="msg-name-input" type="text" name="name" value="' . esc_attr__('Hello', 'disputo') . '" placeholder="' . esc_attr__('Chat name...', 'disputo') . '" /><textarea name="message" id="message" class="'.(bbpm_has_errors('message')?'has-errors':'').'" required="required" placeholder="' . esc_attr__('Type a message..', 'disputo') . '">' . $autosave . '</textarea>';
...
function bbpm_pending_delete($str, $m) {
    if ( $m->classes && in_array('pending-delete', $m->classes) ) {
        $str .= __('&nbsp;<em class="bbpm-inline-tooltip">[pending-delete]</em>', 'disputo');
    }
...
To unsubscribe from all notifications, please navigate to your profile edit screen.', 'disputo')),
        'html_emails' => false,
        'cache_ctx' => 'object',
...
function bbpm_prepare_contact_button($user_id) {
    ...
    if ( !empty($user->ID) ) {
        ...
        $btn['link_title'] = sprintf(__('Send %s a Message', 'disputo'), $user->display_name);
    }

    $btn['inner_text'] = __('Send Message', 'disputo');
